      ...
      <nav id="header" className="w-full z-30 top-0 py-1 border-b-2">
        <div className="w-full container mx-auto flex flex-wrap items-center justify-between mt-0 px-6 py-3">
              ...
              <ul className="lg:flex items-center text justify-between text-base font-bold text-gray-700 pt-4 lg:pt-0">
                {
                  categories.map(({ name }) => (
                    <li key={name} className="bg-gray-100 lg:bg-transparent rounded-md border-b-2 lg:border-b-0 border-gray-200">
                      <Link className="inline-block no-underline hover:text-black rounded-lg lg:hover:bg-gray-200 transition ease-out duration-300 py-2 px-4 capitalize" to={`/${name}`}>{name}</Link>
                    </li>
                  ))
                }
              </ul>
          ...
          <div className="order-2 lg:order-3 flex items-center" id="nav-content">
            ...
            <Link className="pl-3 inline-block no-underline font-bold hover:text-black" to="/cart">
